...
└── docs/
    ...
    ├── 経費証憑OCRツール_要件定義.md                     # 経費証憑OCRツール 要件定義書
    └── 経費証憑OCRツール_設計仕様書.md                   # 経費証憑OCRツール 設計仕様書
```

### 経費証憑 OCR ツール（別用途展開）

本リポジトリのアーキテクチャをベースに、**経費証憑（レシート・領収書など）の OCR 処理と立替経費精算申請の下書き自動登録**への展開ドキュメントを追加しています。

- [`docs/経費証憑OCRツール_要件定義.md`](docs/経費証憑OCRツール_要件定義.md)
- [`docs/経費証憑OCRツール_設計仕様書.md`](docs/経費証憑OCRツール_設計仕様書.md)
